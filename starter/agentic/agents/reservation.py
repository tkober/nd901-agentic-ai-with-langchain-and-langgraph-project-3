...
async def reservation_agent_node(
    ...
    reservation_tools = (
        McpToolFilter(tools).by_tags(["reservation", account_id]).get_all()
    )

    system_prompt = dedent(f"""
        You are an agent that helps users manage their reservations for {account_name} (account_id={account_id}).
        {account_description}
        Your task is to assist users with managing their reservations (e.g. creating, cancelling, changing, etc.). 
        You have access to tools that allow you to manage the users reservations.   
        ...
        - Before you perform any action that changes the users reservation, always ask the user for confirmation. Only after they confirm, perform the action.
